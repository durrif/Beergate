...
async def list_purchases(
    ...
    result = await db.execute(
        ...
        .where(Purchase.brewery_id == brewery.id)
        ...
        if item_data.ingredient_id:
            ing_result = await db.execute(
                select(Ingredient).where(
                    ...
                    Ingredient.brewery_id == brewery.id,
                )
            )
...
async def get_purchase(
    ...
    result = await db.execute(
        select(Purchase)
        .options(selectinload(Purchase.items))
        .where(Purchase.id == purchase_id, Purchase.brewery_id == brewery.id)
    )
...
async def delete_purchase(
    ...
    result = await db.execute(
        select(Purchase).where(Purchase.id == purchase_id, Purchase.brewery_id == brewery.id)
    )
    purchase = result.scalar_one_or_none()
    if not purchase:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Purchase not found")
