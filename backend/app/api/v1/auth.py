...
from app.schemas.brewery import BreweryOut, BreweryCreate
...
async def register(payload: RegisterRequest, db: AsyncSession = Depends(get_db)) -> User:
    """Register a new brewer account with a default brewery."""
    # Check uniqueness
    ...
    # Create default brewery named after user
    brewery = Brewery(
        name=f"Cervecería de {payload.full_name}",
        owner_id=user.id,
    )
    db.add(brewery)
    await db.commit()
...
async def login(request: Request, payload: LoginRequest, db: AsyncSession = Depends(get_db)) -> dict:
    ...
    # Fetch brewery for the user
    brewery_result = await db.execute(
        select(Brewery).where(Brewery.owner_id == user.id)
    )
    brewery = brewery_result.scalar_one_or_none()
    ...
    return {
        ...
        "brewery": BreweryOut.model_validate(brewery).model_dump() if brewery else None,
    }
...
async def get_me_full(
    ...
    """Return user profile + brewery in one call (for app bootstrap)."""
    brewery_result = await db.execute(
...
async def accept_invitation(
    ...
    # Create default brewery
    brewery = Brewery(
        name=f"Cervecería de {user.full_name}",
        owner_id=user.id,
    )
    db.add(brewery)
    await db.commit()
