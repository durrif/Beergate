...
async def call_beergate_voice(text: str) -> str:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        if resp.status_code != 200:
            ...
            return "Lo siento, no he podido procesar tu petición."

        data = resp.json()
        return data.get("reply", data.get("response", "Sin respuesta."))
...
async def process_voice(audio: UploadFile = File(...), skip_trigger: bool = False):
    ...
    # 3. Send to Beergate
    if clean_text:
        response_text = await call_beergate_voice(clean_text)
    else:
