# Beergate v2 — Production Deployment Guide
...
## CT/VMs
- Old Beergate (production): CT 101 — port 8000
- New Beergate v2 (this): CT 102 — frontend :8080, backend :8001

## First-time setup

```bash
# 1. Clone / copy to CT 102
cd /opt/beergate/simple-backend/beergate-v2
...
# 6. (Optional) Migrate legacy data from old beergate
...
```nginx
server {
    server_name beergate.es www.beergate.es;
    listen 443 ssl http2;

    # SSL certs (Let's Encrypt via certbot)
    ssl_certificate /etc/letsencrypt/live/beergate.es/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/beergate.es/privkey.pem;
...
docker compose -f docker-compose.production.yml exec postgres \
  pg_dump -U beergate beergate | gzip > /backup/beergate-$(date +%Y%m%d).sql.gz
```
